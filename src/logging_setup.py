...
SENSITIVE_PATTERNS = [
    ...
    (
        re.compile(r"(ltuid|ltoken|cookie_token_v2|account_id|session_id)\s*=\s*([^;,\s]+)"),
        r"\1=***",
    ),
    ...
    if not _metrics:
        _metrics = {
            "codes_found_total": Counter(
                "genshin_codes_found_total", "Total codes found", ["source"]
            ),
            ...
            "fetch_errors_total": Counter(
                "genshin_fetch_errors_total", "Total fetch errors", ["source", "error_type"]
            ),
            ...
            "active_sources": Gauge("genshin_active_sources", "Number of active sources"),
            "stored_codes": Gauge(
...
class SensitiveDataFilter(logging.Filter):
    ...
    def filter(self, record: logging.LogRecord) -> bool:
        ...
        if record.args:
            record.args = tuple(
                self._mask_sensitive(arg) if isinstance(arg, str) else arg for arg in record.args
            )
...
def setup_logging(config: dict[str, Any] | None = None) -> logging.Logger:
    ...
    file_handler = _create_rotating_file_handler(log_file, max_bytes, backup_count, log_level)
